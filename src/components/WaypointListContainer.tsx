...
function WaypointListItem({ waypoint, selectedId, onClick }: { waypoint: Waypoint; selectedId: number | null; onClick: (id: number) => void }) {
    ...
    useEffect(() => {
        if (containerRef.current && waypointType) {
            ...
            renderMarker(g, {x: 11, y: 11}, false, 10, waypointType, settings.waypointBorderWidth, settings.waypointBorderColor, settings.showWaypointBorder);
        }
